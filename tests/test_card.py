...
def test_thumb_page_prefers_hit(qtbot):
    ...
    qtbot.addWidget(it)
    assert it.findChild(QLabel, "cardThumb") is None
    assert not hasattr(it, "thumb_page")
...
def test_thumb_worker_coalesces_inflight_duplicate_requests(qtbot, monkeypatch, tmp_path):
    ...
    def fake_thumb(path, page_no, long_edge=480):
        with lock:
    ...
    monkeypatch.setattr(thumbnailer, "find_non_com_thumbnail", fake_thumb)
...
def test_thumb_worker_skips_powerpoint_when_background_render_is_blocked(qtbot, monkeypatch, tmp_path):
    calls: list[str] = []
...
def test_thumb_worker_thumbnail_error_emits_failure_and_worker_survives(qtbot, monkeypatch, tmp_path):
    calls: list[tuple[str, int]] = []
...
def test_mainwindow_does_not_request_result_thumbs_on_search(qtbot, tmp_path):
...
def test_mainwindow_legacy_thumb_worker_is_not_part_of_result_rendering(qtbot, tmp_path):
    conn = _index(tmp_path)
    stub = StubThumb()
    win = MainWindow(conn=conn, render_worker=StubRender(), thumb_worker=stub, do_index=False)
    qtbot.addWidget(win)
    win._render_results([_fr()])
    assert stub.requests == []
    assert win.result_list.itemWidget(win.result_list.item(0)).findChild(QLabel, "cardThumb") is None
